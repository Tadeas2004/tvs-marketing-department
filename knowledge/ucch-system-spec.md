...
The Brand Summary page (3bf25cd2-e540-80f7-bd22-e5f3b07bc821) is the page the n8n workflow's "Fetch Brand Context" node pulls via `getMarkdown: page`, feeding a Groq gpt-oss-120b Hook Selector agent alongside 10 candidate hooks for the viral-reel reconstruction pipeline (§5.2). These hooks are opening lines for short-form vertical video specifically — IG Reels and YouTube Shorts, spoken-to-camera/voiceover/text-on-screen — not Threads, X, LinkedIn, or any written-text platform.

Rewritten 2026-09-07 across three passes: (1) cut from ~950 words to ~300, removing pricing tiers, funnel architecture, and a 9-layer internal-pipeline description, and fixing staleness — the closed old digital-product business was presented as active, and the content audience was conflated with the agency's client ICP; (2) cut again to ~150 words after the free-tier Groq model was still choking on token volume, dropping school/employer names and the case-study client detail; (3) restored to ~230 words after pass 2 cut too much real grounding (actual follower numbers, the credibility angle) and, worse, still wrongly listed Threads as part of the audience for this pipeline — Threads is where the 30k-follower text audience lives, but the Creators DB this system scrapes is all Instagram reel creators, and the output platform is IG Reels/YT Shorts only, so Threads has no place in this doc at all. Final version explicitly tells the model never to mention Threads as an output platform for these reels. It now contains:

- **Format note, stated first**: spoken/on-screen video hooks for IG Reels & YT Shorts ONLY, no Threads/X/LinkedIn — judge each one as the first 1-3 seconds before a viewer scrolls past, not as written prose.
- **Identity**: CS student + enterprise AI-eng intern, building the AI automation agency on the side, pre-revenue. Old Threads-growth digital-product business (~$5k in past sales) explicitly marked closed/do-not-pitch, and explicitly not an output channel for this pipeline.
- **Audience**: real current numbers — IG following is small (~500), YouTube hasn't started posting — a growing video audience, not an established one. Skews builders/creators/marketers/AI-curious. Explicitly *not* the agency's SMB client ICP, which is who cold outreach targets instead.
- **Content topics**: AI tools, AI agents, marketing/content automation, build-in-public logs, student/solo-operator life.
- **Voice rules ("Radical Authenticity")**: direct, no fluff, punchy, never overclaims, sounds spoken not written, not a guru — a short say/never-say list (on-brand phrases + banned language, including "never invent follower/revenue numbers").
...
- **An explicit reject list**: no overclaiming revenue/clients/results, no reviving the old digital-product business, no flash-over-substance AI-demo hooks, no written-headline phrasing, no mentioning Threads as an output platform.
