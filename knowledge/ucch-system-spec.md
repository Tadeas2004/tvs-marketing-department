...
The Brand Summary page (3bf25cd2-e540-80f7-bd22-e5f3b07bc821) is the page the n8n workflow's "Fetch Brand Context" node pulls via `getMarkdown: page`, feeding a Groq gpt-oss-120b Hook Selector agent alongside 10 candidate hooks for the viral-reel reconstruction pipeline (§5.2). These hooks are opening lines for short-form vertical video specifically — IG Reels and YouTube Shorts, spoken-to-camera/voiceover/text-on-screen — not Threads, X, LinkedIn, or any written-text platform. Rewritten 2026-09-07 in two passes: first cut from ~950 words to ~300 (removing pricing tiers, funnel architecture, a 9-layer internal-pipeline description, and fixing staleness — the closed old digital-product business was presented as active, and the content audience was conflated with the agency's client ICP), then cut again to ~150 words after the free-tier Groq model was still choking on token volume. The second pass dropped anything that doesn't change which hook gets picked: the specific school/employer names, the case-study client detail, and full-sentence prose in favor of label: value shorthand. It now contains only:

- **Format note, stated first**: spoken/on-screen video hooks for Reels/Shorts only — judge each one as the first 1-3 seconds before a viewer scrolls past, not as written prose.
- **Identity**: one line — CS student + enterprise AI-eng intern, building the AI automation agency on the side, pre-revenue, old Threads-growth digital-product business explicitly marked closed/do-not-pitch.
...
- **Voice rules ("Radical Authenticity")**: direct, no fluff, punchy, never overclaims, sounds spoken not written, not a guru — condensed to a short say/never-say list (on-brand phrases + banned language) rather than full explanatory prose.
