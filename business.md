...
Internship (AI engineering, Spring AI framework, MCP servers, RAG, enterprise AI applications and agents): 3 days/week, 8 hours each, plus ~30 min travel each way.
...
Standalone manual swipe-file pages also live directly under the UCCH root in Notion (Viral Thread Content Examples, Thread Hooks, Instagram Captions, Substack Inspiration) — hand-curated viral posts from the owner and others, saved because they're too good to lose. Not part of the automated pipeline and currently owned by no one; reserved as future context for whichever hire ends up writing Threads/Substack content.
...
- `brand/voice.md`'s first pass (2026-09-12) is thin in two spots: no dictated 2-minute raw-speech sample was collected, and no real cringe-marketing screenshots from the AI-agency space were provided (both categories came back empty in the build-your-voice session). Also missing: a real closing pattern for a full post or a landing/offer page — only openers exist for those so far. Fill these the next time voice gets worked, and once real new-business posts/hooks exist, replace the old Threads-growth dollar/follower claims that seeded "How I Open" with new ones.
